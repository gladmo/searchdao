...
- ☁️ **坐骑系统**：养成型装备，可合成升级，提供持续成长的属性加成
- ⚔️ **历练玩法**：1000关战斗挑战，击败敌人获得修为和筋斗云朵奖励
...
### 主界面
![游戏初始界面](screenshots/mount-training-initial.png)

### 坐骑系统
![坐骑系统界面](screenshots/mount-modal-locked.png)

### 历练玩法
![历练玩法界面](screenshots/training-modal.png)

### 装备对比
![装备对比功能](screenshots/equipment-comparison.png)
...
- **词条系统**：史诗及以上品质装备可拥有特殊词条
  ...
  - 词条数量随装备等级提升：史诗品质21-40级1个，41-60级2个，61-80级3个，81-90级4个，91级以上5个
  - 词条给予额外属性加成，提升战力
  
- **技能系统**：史诗及以上品质的高等级装备可拥有技能
  - **触发技能**（6种）：击晕、暴击、连击、闪避、反击、吸血
  - **抗性技能**（6种）：击晕抗性、暴击抗性、连击抗性、闪避抗性、反击抗性、吸血抗性
  - 技能数量随装备等级提升：史诗品质30-49级1个，50-69级2个，70-89级3个，90级以上4个
  - 技能值会随装备等级缩放，高等级装备的技能效果更强
  - 技能也会增加战力计算
  
- **战力计算**：综合评估角色实力
  - 战力 = 攻击 + 生命/10 + 防御×2 + 敏捷
  - 包含装备属性、词条加成、技能加成及坐骑加成
  - 战力越高，高品质装备掉落率越高
  - 战力影响历练玩法的胜负
...
- **坐骑系统**：养成型装备，提供持续成长
  - 在筑基期（10级）解锁坐骑系统
  - 使用100个筋斗云朵合成初始坐骑
  - 通过筋斗云朵升级坐骑（升级消耗随等级递增）
  - 每级提升基础属性：攻击、生命、防御、敏捷
  - 20级以上可获得特殊词条（最多4个）
  - 30级以上可获得技能（最多4个）
  - 坐骑属性直接加入战力计算
  - **筋斗云朵获取方式**：
...
- **修为槽**：类似体力的资源管理
  - 每次砍树消耗1点修为
  - 每秒自动恢复1点修为
  - 最大修为 = 50 + 等级 × 5（等级越高，修为上限越高）
  - 注：修为槽用于消耗，修为点数用于升级，是两个不同的概念
...
1. **开始砍树**
   - 点击中间的树木图标 🌳 或底部的砍树按钮 🪓
   - 每次砍树消耗1点修为（资源槽），获得10点修为点数（升级用），并获得1件随机装备
